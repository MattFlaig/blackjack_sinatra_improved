...
post '/bet' do
  if params[:bet_amount].nil? || params[:bet_amount].to_i == 0
    @error = "You have to make a valid bet."
    halt erb(:bet)
  elsif params[:bet_amount].to_i > session[:player_pot]
    @error = "Sorry, you don't have that much money (#{session[:player_pot]} Euro)!"
    halt erb(:bet)
  else #happy path
...
post '/game/player/hit' do
  ...
  if player_total == BLACKJACK_AMOUNT
    ...
    loser!("Sorry, #{session[:player_name]}, you busted at #{player_total}.")
  end
...
get '/game/compare' do
  ...
  if player_total < dealer_total
    ...
    tie!("Both #{session[:player_name]} and the dealer stayed at #{player_total}. No winner!")
  end
